...
var DependencyWheelSeries = /** @class */ (function (_super) {
    ...
    DependencyWheelSeries.prototype.translate = function () {
        ...
        this.nodeColumns[0].forEach(function (node) {
            ...
            if (node.sum) {
                var shapeArgs = node.shapeArgs, centerX_1 = center[0], centerY_1 = center[1], r = center[2] / 2,
                    innerR_1 = r - options.nodeWidth, start = startAngle + factor * (shapeArgs.y || 0),
                    end = startAngle +
                ...
                node.linksFrom.forEach(function (point) {
                    if (point.linkBase) {
                        ...
                        var corners = point.linkBase.map(function (top, i) {
                            var angle = factor * top, x = Math.cos(startAngle + angle) * (innerR_1 + 1),
                                y = Math.sin(startAngle + angle) * (innerR_1 + 1), curveFactor = options.curveFactor;
