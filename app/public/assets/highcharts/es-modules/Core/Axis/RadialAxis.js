...
var RadialAxis = /** @class */ (function () {
    ...
    RadialAxis.init = function (axis) {
        ...
        axis.getLinePath = function (_lineWidth, radius, innerRadius) {
            var center = this.pane.center, end, chart = this.chart, r = pick(radius, center[2] / 2 - this.offset),
                left = this.left || 0, top = this.top || 0, path;
        ...
        axis.getPlotBandPath = function (from, to, options) {
            ...
            var center = this.center, startAngleRad = this.startAngleRad, fullRadius = center[2] / 2,
                offset = Math.min(this.offset, 0), left = this.left || 0, top = this.top || 0, percentRegex = /%$/,
                start, end, angle, xOnPerimeter, open, isCircular = this.isCircular, // X axis in a polar chart
                path, outerRadius = pick(radiusToPixels(options.outerRadius), fullRadius),
        ...
        axis.getPlotLinePath = function (options) {
            var axis = this, center = axis.pane.center, chart = axis.chart, inverted = chart.inverted,
                ...
                height = axis.height, isCrosshair = options.isCrosshair, paneInnerR = center[3] / 2, innerRatio,
                distance, a, b, otherAxis, xy, tickPositions, crossPos, path;
            // Crosshair logic
            if (isCrosshair) {
    ...
    RadialAxis.compose = function (AxisClass, TickClass) {
        ...
        addEvent(AxisClass, 'init', function (e) {
            ...
            var inverted = chart.inverted, angular = chart.angular, polar = chart.polar, isX = axis.isXAxis,
                coll = axis.coll, isHidden = angular && isX, isCircular, chartOptions = chart.options,
        ...
        addEvent(AxisClass, 'afterInit', function () {
            var axis = this;
            var chart = axis.chart, options = axis.options, isHidden = chart.angular && axis.isXAxis, pane = axis.pane,
        ...
        addEvent(TickClass, 'afterGetLabelPosition', function (e) {
            ...
            var labelBBox = label.getBBox(), labelOptions = axis.options.labels, optionsY = labelOptions.y, ret,
