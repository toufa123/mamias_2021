...
var CSVStore = /** @class */ (function (_super) {
    ...
    * */
    ...
    function CSVStore(table, options, parser) {
        ...
        var csv = options.csv, csvURL = options.csvURL, enablePolling = options.enablePolling,
            dataRefreshRate = options.dataRefreshRate,
            parserOptions = __rest(options, ["csv", "csvURL", "enablePolling", "dataRefreshRate"]);
        _this.parserOptions = parserOptions;
        _this.options = merge(CSVStore.defaultOptions, {
            csv: csv,
            csvURL: csvURL,
    ...
    CSVStore.prototype.poll = function () {
        var _this = this;
        var _a = this.options, dataRefreshRate = _a.dataRefreshRate, pollingEnabled = _a.enablePolling,
            csvURL = _a.csvURL;
    ...
    CSVStore.prototype.load = function (eventDetail) {
        var store = this, parser = store.parser, table = store.table, _a = store.options, csv = _a.csv,
            csvURL = _a.csvURL;
        if (csv) {
    ...
    CSVStore.prototype.getCSVForExport = function (exportOptions) {
        var useLocalDecimalPoint = exportOptions.useLocalDecimalPoint, lineDelimiter = exportOptions.lineDelimiter,
        ...
        var _a = this.getColumnsForExport(exportOptions.usePresentationOrder), columnNames = _a.columnNames,
            columnValues = _a.columnValues;
        var csvRows = [], columnsCount = columnNames.length;
        var rowArray = [];
        // Add the names as the first row if they should be exported
        if (exportNames) {
            csvRows.push(columnNames.map(function (columnName) {
                return "\"" + columnName + "\"";
            }).join(itemDelimiter));
        }
