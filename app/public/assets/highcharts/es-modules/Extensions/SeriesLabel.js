...
var addEvent = U.addEvent, extend = U.extend, fireEvent = U.fireEvent, isNumber = U.isNumber, pick = U.pick,
...
Series.prototype.getPointsOnGraph = function () {
    ...
    var distance = 16, points = this.points, interpolated = [], graph = this.graph || this.area, node = graph.element,
        inverted = this.chart.inverted, xAxis = this.xAxis, yAxis = this.yAxis,
        paneLeft = inverted ? yAxis.pos : xAxis.pos, paneTop = inverted ? xAxis.pos : yAxis.pos,
...
Series.prototype.checkClearPoint = function (x, y, bBox, checkDistance) {
    var chart = this.chart, onArea = pick(this.options.label.onArea, !!this.area),
        findDistanceToOthers = (onArea || this.options.label.connectorAllowed), leastDistance = 16;
...
Chart.prototype.drawSeriesLabels = function () {
    ...
    chart.series.forEach(function (series) {
        ...
        var colorClass = 'highcharts-color-' + pick(series.colorIndex, 'none'), isNew = !series.labelBySeries,
            minFontSize = labelOptions.minFontSize, maxFontSize = labelOptions.maxFontSize, inverted = chart.inverted,
            ...
            onArea = pick(labelOptions.onArea, !!series.area), results = [];
