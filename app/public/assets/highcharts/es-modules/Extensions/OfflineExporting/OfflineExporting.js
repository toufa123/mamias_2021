...
(function (OfflineExporting) {
    ...
    function downloadSVGLocal(svg, options, failCallback, successCallback) {
        var dummySVGContainer = doc.createElement('div'), imageType = options.type || 'image/png',
            filename = ((options.filename || 'chart') +
                '.' +
                (imageType === 'image/svg+xml' ? 'svg' : imageType.split('/')[1])), scale = options.scale || 1;
        var svgurl, blob, finallyHandler, libURL = (options.libURL || defaultOptions.exporting.libURL),
    ...
    function exportChartLocal(exportingOptions, chartOptions) {
        var chart = this, options = merge(chart.options.exporting, exportingOptions),
            fallbackToExportServer = function (err) {
                ...
                if (svg.indexOf('<foreignObject') > -1 &&
                    ...
                    OfflineExporting.downloadSVGLocal(svg, extend({filename: chart.getFilename()}, options), fallbackToExportServer, function () {
